	...
	try {
		...
		if (category) {
			const { data } = await axios.get(
				`/api/products?category=${category}&keyword=${keyword}&pageNumber=${pageNumber}&limit=15`
			);
			dispatch({
			...
			const { data } = await axios.get(
				`/api/products?keyword=${keyword}&pageNumber=${pageNumber}&limit=15`
			);
			dispatch({
				type: PRODUCT_LIST_SUCCESS,
				payload: data,
...
export const createProductReview = (productId, review) => async (
	...
	try {
		...
		toast.dark('Successfully created review!');
	} catch (error) {
		dispatch({
